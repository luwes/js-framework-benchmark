  ...
  try {
    ...
    try {
      results = await lighthouse(
        `http://localhost:${benchmarkOptions.port}/${framework.uri}/`,
        opts,
        null
      );
      await chrome.kill();
...
function writeResult<T>(res: Result<T>, dir: string) {
  ...
  for (let resultKind of benchmark.resultKinds()) {
    ...
    fs.writeFileSync(
      `${dir}/${fileName(res.framework, resultKind)}`,
...
async function runMemBenchmark(
  ...
  await writeResult(
    { framework: framework, results: allResults, benchmark: benchmark },
    benchmarkOptions.outputDirectory
  );
  return { errors, warnings };
}
...
export async function executeBenchmark(
  ...
  if (benchmark.type == BenchmarkType.STARTUP) {
    ...
    errorsAndWarnings = await runCPUBenchmark(
      framework,
      benchmark,
      benchmarkOptions
    );
  } else {
    errorsAndWarnings = await runMemBenchmark(
      framework,
      benchmark,
      benchmarkOptions
    );
...
export async function performBenchmark(
  ...
  let errorsAndWarnings = await executeBenchmark(
    frameworks,
    keyed,
    frameworkName,
    benchmarkName,

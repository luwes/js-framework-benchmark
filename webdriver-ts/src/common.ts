...
export function loadFrameworkVersionInformation(
        ...
        if (fs.existsSync(packageJSONPath)) {
          ...
          if (packageJSON['js-framework-benchmark']) {
            if (
              ...
              typeof packageJSON['js-framework-benchmark'][
                'frameworkVersion'
              ] === 'string'
            ) {
              result.push(
                new FrameworkVersionInformationStatic(
                  keyedType,
                  directory,
              ...
              result.push(
                new FrameworkVersionInformationError(
                  keyedType,
                  directory,
...
export async function determineInstalledVersions(
  ...
  try {
    ...
    let packageLock: any = (await axios.get(
      `http://localhost:${config.PORT}/frameworks/${framework.keyedType}/${framework.directory}/package-lock.json`
    )).data;
...
export async function initializeFrameworks(
  ...
  for (let frameworkVersionInformation of frameworkVersionInformations) {
    if (
      ...
      frameworks.push(
        (await determineInstalledVersions(
          frameworkVersionInformation
        )).getFrameworkData()
      );
    } else if (

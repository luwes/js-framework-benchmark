...
async function runBench(
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    try {
      let errorsAndWarnings: ErrorsAndWarning = await forkedRun(
        frameworks,
        framework.name,
        framework.keyed,
        benchmark.id,
      ...
      console.log(
        `Error executing benchmark ${framework.name} and benchmark ${benchmark.id}`
      );
    }
  }
...
async function main() {
  ...
  if (args.help) {
    ...
    runBench(frameworks, runFrameworks, runBenchmarks, dir)
      .then(_ => {

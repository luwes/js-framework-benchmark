...
function isKeyedRun(result: any): boolean {
  return (
    result.tradded >= 1000 && result.trremoved > 0 && result.removedStoredTr > 0
  );
}
...
export async function checkTRcorrect(
  ...
  if ('true' != spanAria) {
    console.log(
      "Expected to find 'aria-hidden'=true on span in fourth td, but found ",
      spanAria
    );
    return false;
  }
...
async function runBench(frameworks: FrameworkData[], frameworkNames: string[]) {
  let runFrameworks = frameworks.filter(f =>
    frameworkNames.some(
  ...
  for (let i = 0; i < runFrameworks.length; i++) {
    ...
    try {
      ...
      if (
        frameworkMap.get(framework.fullNameWithKeyedAndVersion).keyed !== keyed
      ) {
        console.log(
          'ERROR: Framework ' +
            framework.fullNameWithKeyedAndVersion +

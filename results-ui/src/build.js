...
const frameworks =
  yargs(process.argv, { array: ['framework'] }).framework || [];

function filterFramework(file) {
  return frameworks.length === 0 || frameworks.some(f => file.indexOf(f) > -1);
}
...
fs.readdirSync(RESULTS_PATH)
  .filter(file => file.endsWith('.json') && filterFramework(file))
  .forEach(file => {
...
Object.keys(libs).forEach(implType => {
  libs[implType] = Object.values(libs[implType]).sort((a, b) =>
...
fs.writeFileSync(
  ...
  'export default ' + JSON.stringify(libs),
  'utf8'
);
...
async function build() {
  ...
  var css = fs
    .readFileSync(__dirname + '/bootstrap-reboot.css', 'utf8')
    .replace(/\/\*[\s\S]+?\*\/\s*/gm, '');

...
function colorClass(factor) {
  ...
  for (var i = 0; i < colorSteps.length; i++) {
    if (i == last || factor < colorSteps[i + 1]) break;
  }
...
function eachEnabledLib(cb) {
  Object.keys(STORE.results).forEach(implType => {
    results[implType].forEach(lib => {
      if (frameworkEnabled(implType, lib.name)) cb(implType, lib);
    });
  });
}
...
function setFactors() {
  ...
  eachEnabledLib((implType, lib) => {
    eachEnabledBench(lib, (benchType, res) => {
      ...
      typeMins[res.name] = clampMin(
        Math.min(typeMins[res.name], res[STORE.metric]),
        STORE.clamp[benchType]
      );
    });
  });
  ...
  eachEnabledLib((implType, lib) => {
    eachEnabledBench(lib, (benchType, res) => {
      var typeMins = mins[implType][benchType];
      res.factor =
        clampMin(res[STORE.metric], STORE.clamp[benchType]) /
        typeMins[res.name];
    });
  });
...
function sortBy(metric, sortDir) {
  ...
  if (benchType == 'name')
    ...
    var cmp = (a, b) =>
      STORE.sortDir * (a.factors[benchType] - b.factors[benchType]);
  else {
    var cmp = (a, b) => {
...
function nextSortDir(metric) {
  if (STORE.sortBy == metric) return STORE.sortDir * -1;
  else return 1;
}

function render(implType, benchType, aggr) {
  if (!anyEnabled('benchmarks', benchType)) return null;

  var enabledFrameworks = STORE.results[implType].filter(lib =>
      frameworkEnabled(implType, lib.name)
    ),
    enabledBenches = Object.keys(benchStruct()[benchType]).filter(num =>
      benchEnabled(benchType, num)
    );
  ...
  return el('table', [
    ...
    el('tr', [
      el('th', { class: 'benchname' }, [
        el(
          'a',
          { href: '#', onclick: [sortBy, 'name', nextSortDir('name')] },
          'Name'
        )
      ]),
      enabledFrameworks.map(lib => el('th', lib.name))
    ]),
    ...
    aggr &&
      el('tr', [
        el('th', [
          el(
            'a',
            { href: '#', onclick: [sortBy, benchType, nextSortDir(benchType)] },
            'slowdown'
          )
        ]),
        enabledFrameworks.map(lib =>
          el(
            'th',
            { class: colorClass(lib.factors[benchType]) },
            lib.factors[benchType].toFixed(2)
          )
        )
      ])
  ]);
...
function toggle(what, type, num) {
  var isEnabled = (STORE.enabled[what][type][num] = !STORE.enabled[what][type][
    num
  ]);
  ...
  if (what == 'benchmarks' && !isEnabled && STORE.sortBy == type + '.' + num)
    sortBy('cpu', 1);
  else sortBy(STORE.sortBy, STORE.sortDir);
...
function setAll(what, type, val) {
  ...
  for (var name in all) all[name] = val;
...
function tablesTpl(implType) {
  if (!anyEnabled('frameworks', implType)) return null;

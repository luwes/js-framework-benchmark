...
fs.copySync(
  'webdriver-ts' + path.sep + 'table.html',
  'dist' + path.sep + 'webdriver-ts' + path.sep + 'table.html'
);
...
function include(name) {
  if (name.indexOf('binding.scala') > -1) {
    console.log(
      'name.indexOf("binding.scala")>-1',
      name.indexOf('/target') > -1,
      name.indexOf('/target/web') > -1,
      name
    );
...
_.each(fs.readdirSync('.'), function(name) {
  if (
    ...
    name[0] !== '.' &&
    excludedDirectories.indexOf(name) == -1
  ) {
...
fs.copySync(
  'stem-v0.2.70-non-keyed/node_modules/babel-polyfill/dist/polyfill.min.js',
  'dist/stem-v0.2.70/node_modules/babel-polyfill/dist'
);
fs.copySync(
  'slim-js-v3.3.0-non-keyed/node_modules/slim-js/src/Slim.js',
  'dist/slim-js-v3.3.0/node_modules/slim-js/src/Slim.js'
);

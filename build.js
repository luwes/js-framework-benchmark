...
let args = yargs(process.argv)
  .usage(
    'npm run build [-- [--framework] [--check] [--skipIrrelevant] [--restartWith] [--benchmarks_only]]'
  )
  .help('help')
  .boolean('check')
  .boolean('fork')
...
var core = args.benchmarks_only
  ? []
  : ['webdriver-ts', 'webdriver-ts-results'].map(f => ['', f]);
...
var notRestarter = ([_, name]) =>
  !name.startsWith(restartWithFramework || undefined);
...
var relevant =
  args.skipIrrelevant && !_.some(core, isDifferent)
...
_.each([].concat(relevant, core), function([dir, name]) {
  ...
  if (
    fs.statSync(fullname).isDirectory() &&
    fs.existsSync(path.join(fullname, 'package.json'))
  ) {
...
_.each(testable, function([dir, name]) {
  ...
  if (
    ...
    exec(
      `npm run selenium -- --count ${args.count} --headless --fork ${args.fork} --framework ${idMap[name]}`,
      {
        cwd: 'webdriver-ts',
        stdio: 'inherit'
      }
